...
public class ReportController implements Initializable {
	...
	void setResult() {
		...
		for (int i = 0; i < list_d.size(); i++) {
				...
				name= "" +   list_d.get(i).filename;
		
			
			v.getChildren().add(
					getVBox(name,list_d.get(i).data.get("30lpmresult").toString(), list_d.get(i).data.get("80lpmresult").toString(),list_d.get(i).data.get("160lpmresult").toString(),clrs.get(i)));
