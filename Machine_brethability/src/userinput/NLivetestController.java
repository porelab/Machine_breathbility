...
//		/setTemp();
		...
		System.out.println("test type  :"+ss);
		if (ss.equals("1")) {
			lbltesttype.setText("Inhalation Test");
		}
		else if (ss.equals("2")) {
			lbltesttype.setText("Exhalation Test");
		}
...
//					}
	...
	void createCsvTable() {
		...
		try {
			...
			List<Double> as=getXPoint(dryplist, dryflist);
			double as30lpm=as.get(0);
			double as80lpm=as.get(1);
			double as160lpm=as.get(2);
			
			
			cs.newLine("30lpm",getRound5(as30lpm));
			cs.newLine("80lpm",getRound5(as80lpm));
			cs.newLine("160lpm",getRound5(as160lpm));
			
			double ffp1=0.00870226,ffp2=0.0101526,ffp3=0.0145038;
			
			
			if(as30lpm<ffp1)
			{
				cs.newLine("30lpmresult","Pass");
				cs.newLine("30lpmcase","FFP1");
				
			}
			else if(as30lpm<ffp2)
			{
				cs.newLine("30lpmresult","Pass");

				cs.newLine("30lpmcase","FFP2");
			}
			else if(as30lpm<ffp3)
			{
				cs.newLine("30lpmresult","Pass");

				cs.newLine("30lpmcase","FFP3");
			}
			else
			{
				cs.newLine("30lpmcase","FFP3");
				cs.newLine("30lpmresult","Fail");
			}
			
			
			double ffp11=0.0304579,ffp21=0.0348091,ffp31=0.0435113;
			...
			if(as80lpm<ffp11)
			{
				cs.newLine("80lpmresult","Pass");
				cs.newLine("80lpmcase","FFP1");
				
			}
			else if(as80lpm<ffp21)
			{
				cs.newLine("80lpmresult","Pass");

				cs.newLine("80lpmcase","FFP2");
			}
			else if(as80lpm<ffp31)
			{
				cs.newLine("80lpmresult","Pass");

				cs.newLine("80lpmcase","FFP3");
			}
			else
			{
				cs.newLine("80lpmcase","FFP3");
				cs.newLine("80lpmresult","Fail");
			}
			
			
			double ffp12=0.0435113;//,ffp22=0.0101526,ffp32=0.0145038;
			...
			if(as160lpm<=ffp3)
			{
				cs.newLine("160lpmresult","Pass");
				cs.newLine("160lpmcase","FFP1");
				
			}
			else
			{
				cs.newLine("160lpmcase","FFP1");
				cs.newLine("160lpmresult","Fail");
			}
			...
			System.out.println("test type  :"+ss);
			if (ss.equals("1")) {
				cs.newLine("testtype","Inhalation");
			}

			else if (ss.equals("2")) {
				cs.newLine("testtype","Exhalation");
			}
	...
	public String getRound5(Double r) {
	...
	List<Double> getXPoint(List<String> x1, List<String> y1)
	{
		...
		{
			xx.add(Double.parseDouble(x1.get(i)));
			yy.add(Double.parseDouble(y1.get(i))*60);  //convert into SSCM from SSCS
		}
		
		PolynomialRegression pp = new PolynomialRegression(yy, xx, 1,"n");
		double[] polycoff = pp.polyfit();
		double lpm30 = pp.polynomial(30000, polycoff);
		double lpm80 = pp.polynomial(95000, polycoff);
		double lpm160 = pp.polynomial(160000, polycoff);
		
		
		System.out.println("Psi on 30lpm  : "+lpm30);
		System.out.println("Psi on 80lpm  : "+lpm80);
		System.out.println("Psi on 160lpm  : "+lpm160);
