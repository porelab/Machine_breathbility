...
public class Singlepororeport {
	...
	Font sampleinfola = FontFactory.getFont(FontFactory.HELVETICA, 15,
			Font.NORMAL, new BaseColor(255, 255, 255));
	...
	Font unitlab = FontFactory.getFont(FontFactory.HELVETICA, 9, Font.BOLD,
			new BaseColor(100, 100, 100));
	Font unitlab1 = FontFactory.getFont(FontFactory.HELVETICA, 14, Font.BOLD,
			new BaseColor(100, 100, 100));
	...
	void coverpage(DatareadN d) {
		...
		PdfPCell r3 = new PdfPCell(
				new Paragraph(d.data.get("testtype").toString().toUpperCase()+" Resistance TEST REPORT".toUpperCase(), testt));
		r3.setBorder(0);
	...
	void sampleinfo(DatareadN d) {
		...
		f33.setBorder(f33.RIGHT | f33.LEFT|f33.BOTTOM);
		...
		f44.setBorder(1);
		f44.setBorder(f44.RIGHT|f44.BOTTOM);
		...
		PdfPCell r1 = new PdfPCell(new Paragraph("30 l/min",
				sampleinfola));
		...
		PdfPCell r1r;
		if (darcy== true) {
			 r1r = new PdfPCell(new Paragraph(d.data.get("30lpmcase")+" "+d.data.get("30lpmresult")+"",
					unitlab));
			}
		else {
		...
		PdfPCell u1;
		if (darcy== true) {
			 u1 = new PdfPCell(new PdfPCell(new Paragraph(""+DataStore.ConvertPressure(d.data.get("30lpm")+"")+" "+DataStore.getUnitepressure(), unitlab1)));
			}
		else {
		...
		//u1.setFixedHeight(30f);
		u1.setBackgroundColor(backcellcoltable1);
		u1.setHorizontalAlignment(Element.ALIGN_CENTER);
		u1.setVerticalAlignment(Element.ALIGN_BOTTOM);



		PdfPCell r12 = new PdfPCell(new Paragraph("95 l/min",
				sampleinfola));
		// r1 = new PdfPCell(new Phrase("jj"+d.starttime));
		r12.setBorder(1);
		...
		PdfPCell r1r2;
		if (gurley== true) {
			r1r2 = new PdfPCell(new Paragraph(d.data.get("80lpmcase")+" "+ d.data.get("80lpmresult")+"",
					unitlab));
			}
		else {
		...
		PdfPCell u12;
		if (gurley== true) {
			u12 = new PdfPCell(new Paragraph(""+DataStore.ConvertPressure(d.data.get("80lpm")+"")+" "+DataStore.getUnitepressure(), unitlab1));
			}
		else {
	...
	//	u12.setFixedHeight(30f);
		u12.setBackgroundColor(backcellcoltable1);
		u12.setHorizontalAlignment(Element.ALIGN_CENTER);
		u12.setVerticalAlignment(Element.ALIGN_BOTTOM);

		
	

		PdfPCell r13 = new PdfPCell(new Paragraph("160 l/min",
				sampleinfola));
		// r1 = new PdfPCell(new Phrase("jj"+d.starttime));
...
//		r13.setBackgroundColor(backcellcoltable);
		...
		PdfPCell r1r3;
		if (frazier== true) {
			r1r3 = new PdfPCell(new Paragraph(""+d.data.get("160lpmcase")+" "+ d.data.get("160lpmresult")+"",
					unitlab));
			}
		else {
		...
		PdfPCell u13;
		if (frazier== true) {
			u13 = new PdfPCell(new Paragraph(""+DataStore.ConvertPressure(d.data.get("160lpm")+"")+" "+DataStore.getUnitepressure(), unitlab1));
			}
		else {
	...
	class HeaderFooterPageEvent extends PdfPageEventHelper {
		...
		private void addHeader(PdfWriter writer) {
			try {
				...
				PdfPCell cell;
				cell = new PdfPCell(new Phrase(d.getStd(), headertestname));
				cell.setBorder(1);
